...
public class HeroService
{
    ...
    public HeroService(Logger logger, HttpClient client)
    {
        _logger = logger;
        _httpClient = client;
    }
    ...
    {
        var requestUri = new Uri(_httpClient.BaseAddress, "heros");
        _logger.Information($"Sending GET request to {requestUri}");

        var response = await _httpClient.GetAsync("heros");
        var responseContent = await response.Content.ReadAsStringAsync();

        if (!response.IsSuccessStatusCode)
        {
            _logger.Warning($"Received non-success status code {response.StatusCode} with content: {responseContent}");
             
            throw new HttpRequestException($"Request failed with status code {response.StatusCode}");
        }

        _logger.Information($"Successfully retrieved heroes data: {responseContent}");

        var result = JsonSerializer.Deserialize<List<Hero>>(responseContent);

        _logger.Information($"HeroService returned {result.Count} heroes");
        return result;
    }

    public async Task<HeroStatsWrapper> GetHeroBenchmarcksAsync(int heroId)
    {
        var requestUri = new Uri(_httpClient.BaseAddress, $"heros/{heroId}/benchmarks");
        _logger.Information($"Sending GET request to {requestUri}");

        var response = await _httpClient.GetAsync($"heros/{heroId}/benchmarks");
        var responseContent = await response.Content.ReadAsStringAsync();
    ...
    {
        var requestUri = new Uri(_httpClient.BaseAddress, $"heroes/{heroId}");
        _logger.Information($"Sending GET request to {requestUri}");

        var response = await _httpClient.GetAsync($"heros/{heroId}");
        var responseContent = await response.Content.ReadAsStringAsync();
